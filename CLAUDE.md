...
**Documented, deliberately-deferred follow-ups** (not bugs): provisional transaction numbering — debt/dashboard numbers derive from type+id; real per-type sequences await the DR-14 ruling. The order `Returned` transition is a bare status flip pending the refund-drives-return model (M5 open note).
