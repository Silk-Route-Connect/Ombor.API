...
The backend is a migration of a substantial legacy implementation to the redesigned model — multi-tenancy (global filter + stamping), audit interceptor, partial WAC, and refund-rule validation already exist; Wallets, the redesigned payment fields, and several read models do not. Per-milestone state lives in `docs/backend-build-plan.md`. Known schema debt at audit time: `Tenant` not yet renamed to `Organization`; `PartnerBalance` not tenant-scoped; `Product.QuantityInStock` still writable; Category→Products cascade-delete; dead `TransactionType.WriteOff`; `GET /api/payments/{id}` throws. All tracked in M0.
