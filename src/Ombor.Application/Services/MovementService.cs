...
internal sealed class MovementService(IApplicationDbContext context) : IMovementService
{
    public async Task<WarehouseMovementDto[]> GetWarehouseMovementsAsync(int warehouseId)
    {
        ...
        movements.AddRange(openings.Select(o => new Raw(
            o.Id, o.DateUtc, "Opening", o.ProductId, o.ProductName, o.Measurement.ToString(), warehouseId, string.Empty, null, null, o.Quantity)));
        ...
        movements.AddRange(adjustments.Select(a => new Raw(
            a.Id, a.DateUtc, "Adjustment", a.ProductId, a.ProductName, a.Measurement.ToString(), warehouseId, string.Empty, null, a.Note ?? a.Reason,
        ...
        movements.AddRange(transferLines.Select(l =>
        {
            ...
            return new Raw(
                l.Id, l.DateUtc, "Transfer", l.ProductId, l.ProductName, l.Measurement.ToString(), warehouseId, string.Empty,
    ...
    public async Task<ProductMovementDto[]> GetProductMovementsAsync(int productId)
    {
        ...
        var openings = await context.OpeningStocks
            ...
            .ToListAsync();
        movements.AddRange(openings.Select(o => new Raw(
            o.Id, o.DateUtc, "Opening", productId, string.Empty, string.Empty, o.WarehouseId, o.WarehouseName, null, null, o.Quantity)));
        ...
        var adjustments = await context.StockAdjustments
            ...
            .ToListAsync();
        movements.AddRange(adjustments.Select(a => new Raw(
            a.Id, a.DateUtc, "Adjustment", productId, string.Empty, string.Empty, a.WarehouseId, a.WarehouseName, null, null,
            a.Direction == StockAdjustmentDirection.Increase ? a.Quantity : -a.Quantity)));
        ...
        {
            movements.Add(new Raw(l.Id, l.DateUtc, "Transfer", productId, string.Empty, string.Empty, l.FromWarehouseId, l.FromName, null, null, -(int)l.Quantity));
            movements.Add(new Raw(l.Id, l.DateUtc, "Transfer", productId, string.Empty, string.Empty, l.ToWarehouseId, l.ToName, null, null, (int)l.Quantity));
        }
